...
const TaskerFlatList = () => {
  ...
  const ItemRender = ({
    ...
    <View style={[styleSheet.item, styles.UIZtmtcx]}>
      <Text style={[styleSheet.itemText]}>{taskName}</Text>
      <View style={[styleSheet.titleText, styles.AnwmTouP]}>
  ...
  const Separator = () => {
    return <View style={styles.IaykfJFx} />;
  };

  return <FlatList data={FILTERS} renderItem={({
    item,
    index
  }) => <ItemRender price={item.price} index={index} taskName={item.taskName} />} keyExtractor={item => item.id.toString()} ItemSeparatorComponent={Separator} horizontal={true} showsHorizontalScrollIndicator={false} />;
};
